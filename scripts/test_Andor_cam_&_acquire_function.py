# -*- coding: utf-8 -*-
"""
Created on Thu Mar 19 12:23:33 2026
...
Nz                       = np.array([7.6])# to move the stage to acquire the third spatial dimension
...
object_name              = 'fluo_µsphere-gel-bin_2' 
data_folder_name         = '2026-03-19_test_SPIM'#'Patient-69_exvivo_LGG_BU'
...
def runCam_thread(cam, acquisition_params, DMD_params, all_path, NR: int = 1, iLc: int = 1, NA: int = 1, first_acqui: bool = True, verbose: bool = False): 
    ...
    # data_np = np.stack(cam.grab(acquisition_params.pattern_amount), axis=0)
    print('grab ' + str(acquisition_params.pattern_amount) + ' images')
    data_np = cam.grab(acquisition_params.pattern_amount)
    print('end of grab')
...
for iNR in range(acquisition_params.NRepetitions):#tqdm(range(acquisition_params.NRepetitions)):
    for iLc in range(len(acquisition_params.Lc)):
        for NA in range(acquisition_params.NAverages):
            ...
            if first_acqui == True:
                if raw_data is not None:
                    raw_data_arr = np.empty((raw_data[0].shape + (acquisition_params.NAverages, 
...
#%% Acquire without thread
shutter.open()
first_acqui = True
verbose = False
cam_spec.setup_acquisition(mode="sequence", nframes = acquisition_params.pattern_amount) 
for iNR in range(acquisition_params.NRepetitions):#tqdm(range(acquisition_params.NRepetitions)):
    for iLc in range(len(acquisition_params.Lc)):
        for NA in range(acquisition_params.NAverages):
            print('start cam')
            ...
            DMD.Run(loop=False)
            print('grab')
            raw_data = cam_spec.grab(acquisition_params.pattern_amount)
            print('end of grab')
            # print(raw_data)
