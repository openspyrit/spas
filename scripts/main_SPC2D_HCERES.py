...
camPar.rectAOI.s32X.value      = 1150#550#0 #0#  // X
camPar.rectAOI.s32Y.value      = 850#380#0#0#800#   // Y
...
camPar = setup_cam(camPar, 
    ...
    ExposureTime = 2,#0.6,# Exposure Time (ms) boudary : [0.013 - 56.221] 
...
pattern_to_display = 'white' #'black'#'gray'#, 
ti   = 4 # Integration time of the spectrometer  
zoom = 1 # Numerical zoom applied in the DMDq
...
if all_path.aborted == False:
    metadata = MetaData(
        ...
        light_source         = source,
        object               = object_name,
        filter               = 'Diffuser',#' + two color filters (green and red)',#optical density = 0.1''No filter',#'linear colored filter',#'Orange filter (600nm)',#'Dichroic_420nm',#'HighPass_500nm + LowPass_750nm + Dichroic_560nm',#'BandPass filter 560nm Dl=10nm',#'None', # + , #'Nothing',#'Diffuser + HighPass_500nm + LowPass_750nm',##'Microsope objective x40',#'' linear colored filter + OD#0',#'Nothing',#
