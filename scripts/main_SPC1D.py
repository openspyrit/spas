...
spectrograph_params = setup_spectrograph(spectrograph,
                                         ...
                                         position    =  634, print_position = True,   # the central wavelength of the grating
                                         unit        = 'nm', print_unit     = True,   # Arg: 'A', 'nm', 'µm'
                                         slit_width  = 125)                          # the width of the slit in (µm)
#%% setup Spatial Camera
cam_spat_params = setup_cam(cam = cam_spat, 
                            cameras_nbr = 2,        # number of camera 
                            data_format = 8,        # format of the data (8 or 16 bits)
                            expos_time  = 0.014,    # [0.001 - 1000] ms
...
DMD_params = play_one_pattern(DMD, DMD_initial_memory, cam_Par = cam_spat_params, zoom = 1, pattern_to_display = 'black') # white, black or gray
...
cam_spec_params = setup_cam(cam = cam_spec, 
                            cameras_nbr = 2,
                            data_format = 16,        # format of the data (8 or 16 bits)
                            expos_time  = 4,       # [0.001 - 1000] ms
                            frame_rate  = 4000,      # maximum is applied, depending of the exposure time 
                            gain        = 18,        # [0 - 18.07] dB
                            gammaY      = 0.31,      # [0.3 - 1]                                   
                            width       = 608,       # [32 - 1280]
...
DMD_params = play_one_pattern(DMD, DMD_initial_memory, cam_Par = cam_spec_params, zoom = 2, pattern_to_display = 'white') # white, black or gray
data = snapshot_cam(cam = cam_spec, binX = 1, binY = 1, disp_bin_effect = False, tilt_image = False) # data_format accepted: 8 or 16 bits
DMD.Halt()
...
xw_offset                = 128+192 # Default = 128
yh_offset                = 0 + 192 # Default = 0
pattern_compression      = 1
pattern_dim              = '1D'
scan_mode                = 'Walsh'  #'Walsh_inv' #'Raster_inv' #'Raster' #
source                   = 'white_LED'#'No source'#'White_Zeiss_lamp'#'Thorlabs_White_halogen_lamp'#'HG-1_Oceanoptics'#No-light'#'Bioblock'#'Laser_405nm_1.2W_A_0.14'#'''#' + white LED might'#
object_name              = 'nothing_ok_12_Lc_' + str(cam_spec_params.gain) + 'dB' #'Ray-912' #'ray_405'#'nothing'   #'Arduino_box_position_1'#'biopsy-9-posterior-margin'#GP-without-sample'##-OP'#
data_folder_name         = '2025-10-31_test_strayline'#'Patient-69_exvivo_LGG_BU'
data_name                = 'obj_' + object_name + '_source_' + source + '_Lc_' + str(Lc[0][0]) + 'nm_Gr_' + str(Lc[0][1]) + '_' + scan_mode + '_im_'+str(Np)+'x'+str(Np)+'_ti_'+str(ti)+'ms_zoom_x'+str(zoom)
...
if all_path.aborted == False:
    ...
    description          = 'cam spacer: 10mm. New DMD holder (24°/7.6mm). for illumination: f=80mm (35mm from object) & f=60mm (80mm from DMD holder), collection: spherical lens f=75mm at 200mm of the DMD and 115mm of the slit + cyl lens at 65mm of the slit'
    ...
    DMD_params = setup_DMD(DMD = DMD, DMD_initial_memory = DMD_initial_memory, acquisition_params = acquisition_params, integration_time = ti, add_illumination_time = 80) 

    if DMD_params.patterns != None:
        print('Total expected acq time  : ' + str(int(acquisition_params.pattern_amount*(ti)/1000 // 60)) + ' min ' + 
              str(math.floor(acquisition_params.pattern_amount*(ti)/1000 % 60)) + ' s ' + str(round((acquisition_params.pattern_amount*(ti+0.044) / 1000 % 1) * 1000)) + ' ms')
else:
    print('setup aborted')
#%% Acquire
# time.sleep(3)
