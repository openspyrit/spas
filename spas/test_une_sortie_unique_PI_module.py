...
class StageParameters:
    array_to_move: Optional[np.ndarray] = None
    class_description: str = "Stage translation PI parameters"

class PIController:
    def __init__(self, model: str = 'C-884', sn: str = '0000000000', verbose: bool = False):
        self.model = model
        self.sn = sn
        ...
        self.axes: List[str] = []
        self.axes_names = ['X', 'Y', 'Z'] # Correspondance pour le verbeux

    def connect(self):
        """Initialise la connexion, les platines, les servos et le Homing."""
        try:
            # 1. Connexion
            self.pidevice.ConnectUSB(serialnum=self.sn)
            if self.verbose:
                print(f"Connecté au {self.pidevice.qIDN().strip()}")
            
            self.axes = self.pitools.getaxeslist(self.pidevice, None)
            ...
            # 5. Attente
            self.pitools.waitontarget(self.pidevice, self.axes)
    ...
    def get_positions(self, verbose: bool = None) -> List[float]:
        """Lit la position actuelle de tous les axes."""
        verb = verbose if verbose is not None else self.verbose
        positions = []
        # On utilise qPOS pour tous les axes d'un coup
        pos_dict = self.pidevice.qPOS(self.axes)
        
        for i, axis in enumerate(self.axes):
            pos = pos_dict[axis]
            if verb:
                name = self.axes_names[i] if i < len(self.axes_names) else axis
                print(f'Current position of {name} = {pos:.4f} mm')
            positions.append(pos)
        return positions

    def move_axis(self, axis: str, target_positions: np.ndarray):
        """Déplace un axe spécifique selon un array de positions absolues."""
        maxrange_dict = self.pitools.getmaxtravelrange(self.pidevice, axis)
        max_limit = list(maxrange_dict.values())[0]

        if np.max(target_positions) > max_limit:
            print(f"Warning: Valeur trop élevée ({np.max(target_positions)}). Limite: {max_limit}")
            return

        for pos in target_positions:
            self.pidevice.MOV(axis, pos)
            self.pitools.waitontarget(self.pidevice, axis)
            if self.verbose:
                print(f'Stage {axis} on target: {pos:.4f} mm')

    def go_to_zero(self):
        """Envoie tous les axes à la position 0."""
        for axis in self.axes:
            self.pidevice.MOV(axis, 0)
        self.pitools.waitontarget(self.pidevice, self.axes)
        print('All axes are set to zero')

    def move_to_middle_with_ui(self):
        """Déplace les axes au centre avec une fenêtre de stop d'urgence."""
        status = {"stop": False, "done": False}

        def emergency_stop():
            status["stop"] = True
            try:
                self.pitools.stopall(self.pidevice)
        ...
        def check_status():
            if status["done"]: root.quit()
            else: root.after(100, check_status)

        def motion_thread():
            try:
                self.pitools.movetomiddle(self.pidevice, self.axes)
                while not status["stop"]:
                    if not any(self.pidevice.IsMoving(self.axes).values()):
                        break
                    time.sleep(0.1)
    ...
    def open_jog_control(self):
        """Lance l'interface de contrôle manuel (Jog) dans un thread séparé."""
        def start_gui():
            gui = PIJogControl(self.pidevice, self.axes)
            gui.run()
        
        threading.Thread(target=start_gui, daemon=True).start()

    def disconnect(self, go_home: bool = True):
        """Déconnecte proprement la platine."""
        if go_home:
            self.go_to_zero()
...
# --- Classe GUI Interne ---
class PIJogControl:
    def __init__(self, pidevice, axes):
        ...
        self.root = tk.Tk()
        self.root.title("Contrôle Manuel PI")
